...
5. Lastly, Flat-Folder finds valid solutions for each connected component of
   ...
   $O\left(2^{(n^2)}\right)$ but if each component has only a polynomial number 
   of solutions, solving each connected component independently can implicitly 
   represent an exponential number of folded states in polynomial time. 
